...
7. `boundary.started` 只能引用该事件可见前缀中的最新、仍为 `OpenTail` 的 user turn；后续追加事件不得追溯性地使一个越过或已终止的 turn 合法。turn reducer 输出最早的 `completion_seq`（inline completion 优先于稍后的显式 marker），boundary reducer 直接消费它。
