...
5. checkpoint 提交后重建普通请求并发送。若 Provider 仍报告 context limit，先写带 v1 recovery intent 的 `response.failed`，再写由该 intent 派生的 `context.limit_reached`，保留 checkpoint 和 pending turn，不能循环 compact，不静默截断。普通 `response.started` 只有在 Provider dispatch 前成功保护冻结的 1 MiB durable-outcome headroom 后才可同步；一次性 admission capability 独占该 reserve，直到写入 completed/failed/aborted 或完整 context-limit pair。任意只保留完整 intent 首条的 crash prefix 都能在 reopen 时从 exact `response.started`、intent 和 context snapshot 唯一补全第二条；只有 started 的 crash prefix 也保证仍有空间写 recovery abort 与 marker。
...
3. 已实现 model-aware context 配置、prepared-request 精确 request-shape 测量、`context.configured` / `context.tools` / `response.started` / `context.limit_reached` 审计、真实 usage 差分锚点，以及 Provider context-limit 的 retry/abandon E2E。Provider dispatch admission 在同步 `response.started` 前保护冻结的 1 MiB durable-outcome headroom；context-limit writer 消费同一个一次性 capability 提交 v1 intent transaction，session-open 能从只有 started、或完整 `response.failed` 加缺失/残缺 audit 行的 crash prefix 恢复且不重复。估算只用于 telemetry 和显式 opt-in compaction planning；普通请求不再被 heuristic 伪装成 hard limit 拦截。
