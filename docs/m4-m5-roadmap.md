...
4. 已完成内核级连续两次真实 `compact_once`，并加入 compaction request-boundary v1-v4 的数据模型、provider-attempt/checkpoint 绑定、fail-closed 纯 reducer、版本化 request-slot 状态机、旧 checkpointed budget terminal 的原子兼容迁移、bound Provider 调用和 session-open 恢复：失败 child 不替换 parent checkpoint，随后以同一候选显式重试可形成合法子链，最终 projection 只使用最新 summary + tail。Agent/CLI pending 管理、projection/history abandon 语义，以及“retry intent 已同步但新 attempt 尚未写入”和“legacy budget migration 已 fsync 但正常 response 尚未开始”两个窗口的跨进程强杀恢复 E2E 已完成；后者由新 CLI 进程继续同一 prompt，且不会重复写 migration intent。
...
- 压缩失败、无候选或压缩后无法达到 target 时终止当前请求；同一 boundary 不循环 compact。Provider context-limit retry 使用既有持久化 intent；compaction boundary v1-v4 独立记录 started/checkpointed/failed/retry/abandon，boundary v4 还可承载严格限定的 legacy budget migration，并始终保持原 user message。Agent/CLI pending 管理、projection/history abandon 语义和跨进程闭环 E2E 已实现。冻结的 source projection 尚不能表达 boundary abandon，因此自动 trigger 默认启用前必须保留“cutoff 不跨 abandoned turn”的 fail-closed barrier，或新增版本化 source projection。
