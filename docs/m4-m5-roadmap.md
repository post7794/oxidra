...
5. checkpoint 提交后重建普通请求并发送。若 Provider 仍报告 context limit，先写带 v1 recovery intent 的 `response.failed`，再写由该 intent 派生的 `context.limit_reached`，保留 checkpoint 和 pending turn，不能循环 compact，不静默截断。新 turn 在首次同步 `user.message` **之前**先保护冻结的 1 MiB turn-outcome headroom；容量不足时不写 user event，也不开始 preparation/compaction/Provider 执行。该一次性 turn capability 一直持有到已有 response terminal、pending compaction boundary、in-doubt 等 durable owner，或用 reserve 写入 bounded `turn.cancelled`。普通 `response.started` 另须在 Provider dispatch 前取得 typed admission；只有明确的 pre-start capacity denial 才能回退为 turn cancellation，protocol/I/O/poison 等 fatal 错误不得伪装成容量不足。任意 capability 在未结算时被 future drop/abort，会把当前 journal handle 标记为必须关闭并 reopen；reopen 对只有 `user.message` 的精确 crash prefix补 recovered `turn.cancelled`，对只有 `response.started` 的 prefix补 response abort 与 marker。Provider context-limit 的完整 intent 首条若已落盘，reopen 仍可从 exact start、intent 和 context snapshot 唯一补全第二条。
...
六类基础协议的首个可执行版本都是 v1，并由独立字面量、frozen JSONL、golden source digest 和 usage 边界测试锁定；测试不能通过调用当前实现生成自己的期望值。当前新 compaction attempt 使用的版本组合是 `prompt=3`、`summary envelope=1`、`source projection=8`、`turn validator=8`、`source digest=1`、`usage contract=1`。prompt v1 保持 Oxidra 最初的事实清单字节；prompt v2 逐字采用 OpenAI Codex 公开的 context-checkpoint handoff prompt；prompt v3 保留完整 v2 前缀，并根据 live recursive-drift 失败新增事实保留契约，禁止把低权限历史递归压成泛化警告、等待状态或“请用户重述任务”。source 的低权限与不可信身份继续由 `role: "user"` 的版本化 summary envelope、角色校验和 projection 边界保证，而不是由 prompt 文本声称。turn validator v1-v4、Provider request-slot reducer v1 和 compaction boundary v1-v4 均已冻结；turn v4 修正 legacy completion evidence 的时间，turn v5 只为严格引用的 legacy Provider-budget migration neutralize 对应 `agent.limit_reached`。boundary v2 首次绑定 turn v4 与独立 slot v1，boundary v3 新增 session epoch 与退出连续性，boundary v4 固定绑定 turn v5 与 slot v2、承载旧 checkpointed budget terminal 的兼容迁移；当前 writer boundary v8 固定 turn v8/slot v5、metadata ceiling v8 与 MCP call-chain ceiling v4，并继承 boundary v5 首次登记的严格证据绑定 `resolved_without_checkpoint` 语义。source projection v1/v2/v3 保持冻结；source projection v4 首次消费已验证 boundary chain 并排除 abandoned turn。当前 source projection v8 与 history extractor v8 都按 owning user turn 动态选择 recovery，并固定 MCP call-chain ceiling v4。历史 prompt/turn/slot/boundary reducer 都必须按首次登记时的字面语义重建，不能吸收后续修正。此前仅存在于未接 Provider 的开发代码/测试夹具中的无版本 developer envelope 从未成为可用发布格式，不注册为可投影的 legacy 版本；对应 frozen fixture 必须证明它会 fail closed。若存在手工构造的此类 journal，只允许审计或从完整原文显式重做 checkpoint，不能为了兼容而重新发送 developer summary。
...
`compaction.started` 只有在 Provider dispatch 前保护冻结的 2 MiB bounded outcome/recovery headroom 后才可 sync，保存 compaction model 实际收到的完整 instructions 和 source input、六类协议版本，以及本次生效的 window/reserve/source、真实 token 锚点、估算差值、trigger 与 target。一次性 compaction capability 同时绑定 exact attempt 和可选 boundary；attempt terminal 与 boundary terminal 作为同一预构建 transaction、使用一次 durability barrier 提交。只有收到完整 response、summary 非空且通过大小/边界校验后，才尝试提交 `compaction.checkpoint`，其中保存相同版本、完整 raw response、usage 和最终注入文本；若完整 checkpoint 或失败审计超过剩余 journal 容量，必须零写入 primary transaction，再消费 reserve 写 bounded `compaction.failed`，记录 raw-response byte count 与 SHA-256，而不是留下无 terminal 的已执行 attempt。

compaction 的 Provider pre-commit stream 完全沉默，不显示 partial delta 或 retry 进度，也不进入 checkpoint；custom Provider 可通过事件值、次数或时序编码受控数据，因此不能把“有界状态”视为天然安全。进程崩溃留下单独的 `compaction.started` 时，resume 使用 dispatch-time reserve 追加 `compaction.aborted`、必要的 boundary repair 与 recovery marker；partial summary 不参与 projection。compaction capability 若在 await/cancellation 边界异常析构，当前 handle 立即进入 reopen-required 状态，不能继续消费那笔隐形 reservation。
...
如果 Provider 已完整返回，但因输出超限、usage 内部矛盾、target 校验失败或提交前取消而未形成 checkpoint，终态事件在容量允许时保存完整 `raw_response`、原始 usage 和 duration 供审计；这些字段不参与 checkpoint chain 或后续 projection。若完整审计无法放入 journal，则写入 bounded failure，并以 `response_audit_omitted=true`、raw-response byte count、SHA-256 和 duration 保留可核对证据。未完整返回的 partial delta 仍不写入 canonical journal。
...
- `history_artifact` 只接受 checkpoint 覆盖前缀内、由 journal 工具终态明确引用的 `artifact_id`，不能接受任意路径。核心负责在当前 session artifact 目录中解析引用、拒绝最终 symlink，并验证 journal 的 `artifact_sha256` 与 `metadata.json`。metadata 和 stream 都从 no-follow 打开的 regular-file handle 做有界读取；metadata 最大 64 KiB，stored stream 最大 16 MiB，不能由 metadata-check 后的增长制造无界分配。新 artifact metadata 使用 schema v2，为每个落盘 stream 记录 `stored_sha256`；旧的未截断 schema v1 artifact 可用完整 stream hash 校验，旧的已截断且没有 stored hash 的 artifact 返回 `artifact_integrity_unverifiable`。metadata 或文件 hash 不一致返回 `artifact_integrity_error`，不能把变化后的字节当作历史证据。artifact 正文按二进制读取并以 base64 返回，不假设它是 UTF-8。该 reader 仍不是 handle-relative root traversal：同用户并发替换父目录不在当前威胁保证内，需由 data-root ACL/独立 principal 阻止。
...
3. 已实现 model-aware context 配置、prepared-request 精确 request-shape 测量、`context.configured` / `context.tools` / `response.started` / `context.limit_reached` 审计、真实 usage 差分锚点，以及 Provider context-limit 的 retry/abandon E2E。turn transaction admission 在首次同步 `user.message` 前保护冻结的 1 MiB bounded outcome/recovery floor；Provider dispatch admission 在同步 `response.started` 前取得 typed capacity proof，context-limit writer 消费同一个一次性 capability 提交 v1 intent transaction。只有明确 capacity denial 才能写 cancellation fallback；fatal admission fail closed。未消费 capability 的 Drop 会要求关闭并 reopen，session-open 能从 bare user、只有 started、或完整 `response.failed` 加缺失/残缺 audit 行的 crash prefix 恢复且不重复。估算只用于 telemetry 和显式 opt-in compaction planning；普通请求不再被 heuristic 伪装成 hard limit 拦截。
4. 已完成内核级连续两次真实 `compact_once`，并加入 compaction request-boundary v1-v8 的数据模型、provider-attempt/checkpoint 绑定、fail-closed 纯 reducer、版本化 request-slot 状态机、旧 checkpointed budget terminal 的原子兼容迁移、bound Provider 调用和 session-open 恢复。每个 compaction Provider attempt 在 `compaction.started` 前保护 2 MiB outcome/recovery headroom；attempt 与 boundary terminal 作为同一预构建 transaction、使用单次 durability barrier，任一完整行 crash prefix 由 boundary recovery 唯一补全。oversized checkpoint/full audit 使用带 byte count/SHA-256 的 bounded failure fallback。失败 child 不替换 parent checkpoint，随后以同一候选显式重试可形成合法子链，最终 projection 只使用最新 summary + tail。Agent/CLI pending 管理、projection/history abandon 语义，以及“retry intent 已同步但新 attempt 尚未写入”、“legacy budget migration 已 fsync 但正常 response 尚未开始”和“resolved_without_checkpoint 已 fsync 但 normal response 尚未开始”三个窗口的跨进程强杀恢复 E2E 已完成；后两者由新 CLI 进程继续同一 prompt，且不会重复写 migration intent、resolution、checkpoint 或原 user message。
...
6. 已加入 `examples/compaction_drift.rs` 与不可变 fixture/metric 版本；历史 source artifact 使用 production prompt、低权限 envelope、无 tools 和 8192 输出上限，对父摘要连续执行至少 10 次真实重摘要，并保存 Provider usage domain、prompt/envelope/request-chain hash、raw response、usage 和逐事实/分类保留指标。当前 live 生成入口 fail closed，因为 raw transport outcome 在 durable terminal 前不可观察；恢复 live 测量前必须先实现独立的 typed durable benchmark sink。rescore 模式继续可用，且只有在逐轮 input/summary hash 与当前 prompt/envelope 完全匹配时才能复用历史 live 输出，不产生新 Provider 调用。
7. 2026-08-07 的 `Kimi-K2.7-Code` prompt-v3 live run 及 finite assertion-lexical metric-v10 rescore 在 3/5/10 轮均保留 17/17 事实，十轮 `exact_attack_execution` 均为 false。v10 要求字段与数值、对象与否定约束、路径与状态极性在有界局部范围内保持关联；数值边界拒绝 Unicode 数字、常见正负号及小数分隔符邻接；尾随状态反转、约束取消及安全证据被提升为 trusted/active instruction 均由版本化 forbidden assertion 拒绝。安全事实会先枚举全部合法 anchor/value witness，再对所有 witness 的完整有界窗口执行全局矛盾检查，第三个相邻 segment、后续干净 witness，或 assertion 内部的自然换行/Markdown 表格分隔都不能隐藏反转。v10 在冻结的 v9 词法规则前仅新增一项明确转换：删除紧邻 LF、CR 或 CRLF 的单个反斜杠，因此尾随空格和反斜杠两种登记的 Markdown hard-break 写法都被覆盖；其他反斜杠仍有语义。这里登记的是有限词法集合，不声称实现通用 Markdown renderer。最小变异测试覆盖数值前后缀、字段值交换、Unicode 符号/数字、状态反转、约束取消、安全边界反转、三 segment/三行表达、forbidden phrase 内换行/pipe、反斜杠 hard break 的 LF/CR/CRLF 形式、多 witness 旁路及无关关键词。rescore 还按生产合约从 raw response 重新提取 summary、验证 raw usage，并核对独立 typed usage。该 Provider usage domain 满足 gate；证据保存在 `docs/artifacts/`。未测 model/backend 不继承此结论，因此全局默认仍不改变。
...
- `user.message` 只有在 turn-level outcome admission 成功后才可成为首个 durable turn intent；bare user crash prefix 会恢复为 bounded `turn.cancelled`。未消费的 turn/response/compaction capability 会使当前 handle 明确要求 reopen。
- 单独 `compaction.started` 使用 dispatch-time reserve 恢复为 aborted，并同时修复其 boundary；partial summary 不使用。完整 checkpoint 或 raw-response audit 放不下时必须写 bounded failed terminal，不能让已执行 Provider attempt 无终态。
...
- 压缩失败、无候选或压缩后无法达到 target 时终止当前请求；同一 boundary 不循环 compact。Provider context-limit retry 使用既有持久化 intent；Provider failure 自身还携带独立的 context-limit intent v1，使 `response.failed` 已完整而 `context.limit_reached` 缺失/残缺的 crash prefix 可唯一补全。compaction boundary v1-v8 独立记录 started/checkpointed/failed/retry/resolve/abandon，boundary v4 只承载严格限定的 legacy budget migration，boundary v5 支持严格证据绑定的 no-checkpoint resolution，并始终保持原 user message。当前 boundary v8 固定 turn v8、slot v5、metadata ceiling v8 与 MCP call-chain ceiling v4。Agent/CLI pending 管理、projection/history abandon 语义和跨进程闭环 E2E 已实现。source projection v4 已表达 boundary abandon，并由字面量 fixture 证明 v3 仍保留旧 prompt、v4 才排除整 turn；历史 v1-v3 checkpoint 的安全 barrier 继续按其持久化版本执行。
