...
4. 已完成内核级连续两次真实 `compact_once`，并加入 compaction request-boundary v1/v2/v3 的数据模型、provider-attempt/checkpoint 绑定、fail-closed 纯 reducer、版本化 request-slot 状态机、bound Provider 调用和 session-open 恢复：失败 child 不替换 parent checkpoint，随后以同一候选显式重试可形成合法子链，最终 projection 只使用最新 summary + tail。仍缺 Agent/CLI 对 pending/retry/abandon 的消费，以及覆盖 boundary 与 history 回查的进程级闭环 E2E。
