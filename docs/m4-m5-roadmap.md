...
7. boundary v1 保持首次登记时的接受/拒绝语义。boundary 版本不能只由后写入的 boundary payload 自选：session 内 user turn 协议版本必须按 durable 前缀单调不降，legacy/turn v1-v3 可以继续使用 v1 或升级到 v2，一旦出现 turn v4，后续 user 不得回退到 v1-v3/legacy，且 boundary 最低必须使用 v2；retry 迁移表固定为 `v1 -> v1/v2`、`v2 -> v2`，禁止降级。新 boundary v2 只能引用该事件可见前缀中的最新、仍为 `OpenTail` 的 user turn，并额外消费独立、版本化的 Provider request-slot reducer v1。该 reducer 逐事件验证单 Provider slot：不能并发 `response.started`，上一 response 产生的 function call/tool lifecycle 未全部解决前不能启动下一 response，且 retry 必须从已验证的 terminal 状态重新取得 slot。`OpenTail` 只表示 turn 尚未完成，不是 Provider dispatch 的权限证明；boundary v2 durable 后即保留该 Provider request slot，在 boundary checkpointed 前不得插入普通 response/tool lifecycle。checkpointed 只释放 compaction 对 slot 的独占，随后直到 turn completion 的每个普通 Provider/工具前缀仍必须通过同一个 request-slot reducer，完成证据出现时 slot 必须为 `Terminal`。从 checkpointed 执行 abandon 也必须先证明 slot 为 `Ready` 或 `Terminal`；已有 compaction Provider attempt 的 started boundary 必须先通过 failed/checkpointed marker 结算，不能直接 abandon 隐藏 attempt。后续追加事件不得追溯性地使一个越过、正在请求或已终止的 turn 合法。turn v4 输出最早的 completion evidence `completion_seq`（inline completion 用 response seq，显式 marker 用 marker seq，legacy turn 只有在下一条 user.message 出现时才用该 user seq），boundary v2 直接消费它。
