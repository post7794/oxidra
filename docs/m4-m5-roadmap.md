...
4. 已完成内核级连续两次真实 `compact_once`：失败 child 不替换 parent checkpoint，随后以同一候选显式重试可形成合法子链，最终 projection 只使用最新 summary + tail。仍缺自动 preflight 接入后的 compaction-failure pending boundary，以及覆盖该边界和 history 回查的进程级闭环 E2E。
