...
partial delta 只显示状态，不进入 checkpoint。进程崩溃留下单独的 `compaction.started` 时，resume 使用 dispatch-time reserve 追加 `compaction.aborted`、必要的 boundary repair 与 recovery marker；partial summary 不参与 projection。compaction capability 若在 await/cancellation 边界异常析构，当前 handle 立即进入 reopen-required 状态，不能继续消费那笔隐形 reservation。
