...
状态：M0 核心与 M1 日用 CLI 已实现，并已在 Windows MSVC 与 Windows GNU 的 Rust 1.85 上完成 check、test、Clippy 和 fake-runtime 验收；仓库已配置 CI 和 Windows Release workflow，远端运行仍受 GitHub 账户 Billing 状态阻塞。
...
- read -> edit/write -> shell 工具闭环；edit 调用前展示精确 replacement diff。
...
- 跨平台进程树终止。
- `doctor`、`session list/show`、`trust list/revoke` 管理命令。
- Windows 校验和安装脚本及 GitHub Release workflow。
...
- MVP 只处理 UTF-8 文本；delete/move 后置。

调用执行前，CLI 在 stderr 展示基于 `old_text`/`new_text` 的精确 replacement diff；它用于让用户看清即将发生的替换，不伪装成带文件上下文的完整 unified diff。
...
- 只创建项目根目录内的新 UTF-8 文件，拒绝覆盖已有文件或符号链接。
- 父目录必须已经存在；路径必须是项目内相对路径，并对父目录做 canonical path 校验。
- 内容上限 16 MiB。
- 先写同目录临时文件并 `sync_all`，再以 no-clobber hard link 发布；若平台或文件系统不支持安全发布则明确失败，不降级为可能覆盖目标的操作。
