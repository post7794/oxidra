...
2. 已加入 `examples/compaction_drift.rs`、不可变 fixture/metric 版本和 production-validated rescore。`Kimi-K2.7-Code` prompt-v3 的 live source artifact 与 relation-aware metric-v5 派生 artifact 保存在 `docs/artifacts/`；3/5/10 轮均为 17/17，十轮注入执行均为 false。v5 会验证字段/数值关联、否定约束对象和状态 polarity，rescore 会从 raw Provider response 重新提取 summary 并核对 usage；未测模型不继承该结果。
