...
2. 已加入 `examples/compaction_drift.rs`、不可变 fixture/metric 版本和 production-validated rescore。`Kimi-K2.7-Code` prompt-v3 的 live source artifact 与 assertion-lexical metric-v9 派生 artifact 保存在 `docs/artifacts/`；3/5/10 轮均为 17/17，十轮注入执行均为 false。v9 会验证字段/数值关联与 Unicode-aware 数值 token 边界，拒绝局部状态反转、否定约束取消和安全证据的 trusted/active 极性提升；安全关系的全部合法 witness 共享一个全局有界矛盾检查，第三 segment、额外正向 witness，或 forbidden assertion 自身的换行/表格分隔都不能隐藏反转。rescore 会从 raw Provider response 重新提取 summary 并核对 usage。未测模型不继承该结果。
