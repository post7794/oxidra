...
`--max-responses` 是按 Provider 调用计数的每回合保险丝；开启自动压缩时，内部 compaction Provider 调用也消耗一次额度，随后才允许发送可见 assistant response。可见的 response 数量不包含这次内部调用，但同一进程成功完成的 `TurnOutcome.usage` 会累计 checkpoint 返回的 compaction usage。
