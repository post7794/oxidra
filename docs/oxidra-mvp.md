...
```text
API_KEY       环境变量或 [provider].api_key
API_BASE_URL  环境变量或 [provider].api_base_url；默认 https://api.openai.com/v1
MODEL         环境变量或 [provider].model；默认 gpt-5.6-sol
```

用户配置文件位于平台用户配置目录的 `oxidra/config.toml`；Windows 为 `%APPDATA%\oxidra\config.toml`。环境变量优先于配置文件。兼容回退组为 `OPENAI_API_KEY`、`OPENAI_BASE_URL`、`OPENAI_MODEL`，三者作为整组使用，不与主配置交叉拼接。`[provider].api_key` 是明文存储，不应提交到仓库或分享。
