...
状态：M1-M3 已实现；M5 checkpoint 协议、低权限版本化 summary envelope、真实 Provider `compact_once`、checkpoint-aware Agent projection、受控历史回查、model-aware context 测量/审计、Provider context 超限后的显式 retry/abandon 恢复，以及 `--experimental-auto-compact` preflight 已经实现。当前改动已通过本地 Windows 验证；推送后仍需由 Linux、macOS 和 Windows 远程 CI 重新确认。主线还包含 MCP stdio kernel/config/schema/registry 与 durable coordinator/journal policy core，但尚未接入 CLI、Agent tool surface 或用户 approval，因此不是用户可用的扩展入口。2026-08-07 的 `Kimi-K2.7-Code` prompt-v3 live baseline 在 3/5/10 轮均保留 17/17 事实且未执行注入文本；该证据只绑定记录的 Provider usage domain，其他当前/默认模型未测，因此自动 compaction 继续显式 opt-in。

MCP 首次发布前，尚未进入 durable journal 的 kernel/config/schema/registry identity 已压平成各自单一的 v1 policy；已经写入 journal 的 coordinator/call-chain 等协议按其真实版本保留，不伪装成不存在的历史兼容层。完整接入顺序见 `docs/mcp-roadmap.md`。
...
- 交互式 REPL 与 `-p` 单次模式。
- Responses SSE 在 `response.completed` durable commit 之前保持静默；提交后展示最终文本与已持久化的工具调用投影。
...
- Windows Job Object 与 Unix process group 的常规 shell 子进程清理；MCP 另行要求
  ...
  process group 或启动后 `/proc` sweep 当成强边界。Windows MCP child 由单线程 guardian
  使用 `JOB_LIST`/`HANDLE_LIST` 从 process birth 起原子创建并包含。session-bound guardian 在
  READY 前 fsync durable active-generation record，只有全部已注册 pidfd target 退出或 Job 报告
  无 active process 后才 fsync matching clean record 并释放 gate；guardian 自身先被强杀时，后续
  reopen 会进入永久 quarantine，牺牲恢复可用性而不允许重叠 generation。v1 不支持原地
  recovery；`oxidra session export <ID> <ARCHIVE>.oxidra-session-export` 只能在取得普通 session lock 后写入版本化
  non-journal manifest 和未经 repair 的 exact 原始 JSONL bytes，且不会清除 gate、恢复 session
  或授权 dispatch；manifest 会记录不完整 crash-tail 的 offset/长度/digest，archive 本身不能被
  `SessionStore` 识别为 journal。destination 父目录必须由 operator 控制，不能允许不可信并发
  writer；v1 pathname publisher 不防御同用户 namespace race。它不把
  同用户 MCP 变成低权限插件，主动攻击 guardian/gate 仍需独立 OS 权限边界解决。
- 本地 session journal 与 `--resume`。
- 可审计全局 memory 与 `remember`。
- `doctor`、`session list/show/export/delete`；`session export` 是 quarantined session 的只读归档
  出口，不是 clone/resume/recovery。
- Windows 安装脚本与 Release workflow。

暂不实现：

- 通用扩展/插件安装器，以及 MCP 的 CLI、Agent、approval、journal 用户闭环。底层
  stdio config/schema/registry 与 durable coordinator/journal policy 已实现，但不能据此宣称 MCP 已可用。
...
只允许解析后仍位于项目根内的 canonical path；相对路径中的 `..`、项目根内的绝对路径都按最终 canonical 结果判断，拒绝解析到根外的符号链接或绝对路径逃逸。读取 exact handle 后还会验证 regular-file 类型，并通过 `limit + 1` 有界读取执行 16 MiB 上限，避免 FIFO/device 阻塞和 metadata 后增长造成无界分配；默认返回最多 2000 行或 50 KiB。

当前 project/session/artifact/memory 路径仍由 pathname API 解析，不是 handle-relative capability。支持的平台会在最终文件 open 时使用 no-follow 并按已打开 handle 验证类型，但父目录仍可能在 canonicalize 后被同用户并发 writer 替换。因此项目根、session 数据根和 export 目标父目录必须由 operator 控制；这些检查防止普通模型参数直接逃逸，不声称隔离主动对抗的同 OS principal。Unix 会把数据目录 mode 收紧到 `0700`、durable 文件收紧到 `0600`，但不会枚举或删除额外 POSIX/extended ACL，也不认证远程文件系统的权限语义；Windows 依赖所选 data root 的继承 ACL。需要对抗性路径保证时必须使用独立 principal/sandbox 或 handle-relative broker。
...
- `old_text` 必须恰好匹配一次。
- 初次读取或发布前复查发现 hash 变化时返回 `stale_file`；portable pathname API 不提供“比较 hash 并替换”的单一原子操作，因此复查到 rename 之间的同用户并发修改仍可能被覆盖，项目目录不能有不可信并发 writer。
- 同目录临时文件加原子替换，并保留权限。
...
每次启动追加 `context.configured`，记录实际 model、Provider usage domain、window/reserve/usable/trigger/target、字段来源和测量协议版本。每个启动 epoch 与 tools schema 变化时追加 `context.tools`；每次 `response.started` 保存完整 prepared-request digest、序列化请求字节数、确定性估算、journal/checkpoint/instructions/tools 引用和 usage anchor 差分。MCP-owned call-chain v4 还保存 versioned canonical Provider body envelope；只有 crate 内 sealed 的 MCP exact-wire transport 发送 admission 已封存的同一份 body bytes，公开 custom prepared transport 不属于该类型保证。存在可比较的上一普通 response 时，下一次输入估算使用真实 `input_tokens + E(current) - E(anchor)`；cached tokens 不扣除。无可比较 usage 时才从零估算完整请求。anchor 产生非正值或异常漂移时回退完整请求估算，不 clamp 为 `0`。

当前没有 model tokenizer 或 Provider 计数接口，因此估算只用于显示、审计和显式启用的 compaction planning，不能作为普通请求的 token hard limit。新 turn 在首次 `user.message` fsync 前先取得冻结的 1 MiB turn transaction admission；容量不足时不写 user event，也不开始 preparation、compaction 或 Provider dispatch。该 capability 持续保护 bounded pre-dispatch cancellation/recovery 空间，直到 response terminal、pending boundary、in-doubt 等 durable owner 接管，或写入 `turn.cancelled`。实验开关开启且估算达到 trigger 时，Agent 先同步 boundary intent，再从同一 journal/config 快照为安全 cutoff 构造完整 summary-budget + tail 请求；候选估算包含当前 instructions、实际 tools/history schema 和与 8192 输出上限对应的估算器占位预算，真实 summary 返回后还会按同一 prepared-request/usage-anchor 语义重建，只有达到 target 才提交 checkpoint。同一 user turn 最多自动压缩一次；无候选、Provider 失败或真实 summary 仍超 target 都停止当前请求，不发送原大请求。Provider attempt 已 durable 的失败可由 `--retry-pending` 重放；尚未产生 candidate/attempt 的 preflight-only failure 也由 `--retry-pending` 重新规划，或显式 abandon。planning v1 使用冻结 reader 和字面量 fixture，只把旧 context 当成 lineage 审计证据；resume 时必须按当前 model/context 配置、instructions、tools 和 history view 重新构造完整请求并测量。若当前请求已低于 trigger，boundary v5 要求 retry intent 中的 planning-v1 prefix、estimate 和 trigger 与随后同步的 `compaction.boundary.resolved_without_checkpoint` 逐字段一致，并证明未启动 compaction Provider attempt、slot 仍为 `Ready` 且 `estimate < trigger`；随后不调用 compaction Provider、不生成 checkpoint、不复制 user message，直接继续原 turn。resolution fsync 后崩溃时，`--retry-pending` 复用同一 durable resolution 继续 normal Provider request。不开启实验开关时，普通请求仍直接交给 Provider。每次普通 Provider dispatch 前，Oxidra 先取得 typed outcome admission，再同步 `response.started`；只有明确的 pre-start capacity denial 才能消费 turn reserve 写 bounded cancellation，protocol/I/O/poison 等 fatal 错误零伪造、fail closed。Provider 返回受识别的结构化 context-limit 错误时，同一 capability 提交带独立 error-profile v1 intent 的 `response.failed` 与反向引用该 intent 的 `context.limit_reached`，禁止继续追加新 prompt；若 crash 只有 started，reserve 足以写 recovery abort/marker；若只留下完整 intent 首条或残缺第二条，session-open 会从 exact start/context 补全且不重复。compaction Provider dispatch 同样必须在 `compaction.started` 前保护冻结的 2 MiB outcome/recovery headroom；attempt terminal 与 boundary terminal 作为同一预构建 transaction、使用单次 durability barrier，任一完整行 crash prefix 由 boundary recovery 补全。完整 checkpoint 或 raw audit 放不下时改写带 response byte count/SHA-256 的 bounded failure。turn/response/compaction capability 若在 future timeout/drop/abort 中未消费，当前 journal handle 会明确要求关闭并 reopen，再由统一 recovery 根据实际完整 prefix补 terminal。`--retry-pending --resume <ID>` 从一份 journal snapshot 生成版本化恢复计划：若同 turn 同时存在 context pending 与 failed compaction boundary，先恢复 boundary；历史 durable candidate 按事件记录的协议版本重放；retry intent、compaction Provider 调用和 checkpoint commit 之前，先验证 prospective boundary/turn/slot、checkpoint chain、实际 history snapshot、quota、tools 与后续请求测量。纯 context-limit 同步版本化 `turn.retry_started`；处于 `Ready` Provider slot 的 checkpointed 或 resolved-without-checkpoint boundary 继续原 turn；有 durable candidate 的 failed boundary 同步 `compaction.boundary.retry_started` 后重放压缩。replacement retry intent 同步后、candidate planning 前崩溃时，session reopen 会把孤立 boundary 结算为 failed，下一次 retry 可沿同一 lineage 继续。对于 `55e5b0c` 已写出的 `checkpointed -> agent.limit_reached` 污染状态，只有在当前 `--max-responses` 已提高到大于 durable dispatch 数或被关闭时，才同步原子的 `compaction.boundary.budget_retry_started`：它严格引用旧 limit、旧 boundary 与同一 checkpoint，把 boundary v3 升到 v4。boundary v4、turn validator v5 和 Provider request-slot reducer v2 共同消费一个从 durable prefix 完整验证 lineage、无版本旧 terminal、dispatch 数与恢复状态的 canonical migration 结果；带未知 `provider_call_budget_version` 的 terminal 不会被当成 legacy。boundary v4 还把 turn metadata compatibility ceiling 固定为 v5，因此未来 turn 版本不会追溯改变旧 boundary 的读取语义。旧 turn v4、slot v1 与 boundary v3 的历史结果不变。checkpointed 后的其他 terminal outcome 尚无同 turn retry 协议，必须显式 abandon；Agent 在追加新 `response.started` 前拒绝非法重试。`--abandon-pending --resume <ID>` 同时处理两类 pending，允许用户随后提交替代 prompt；所有路径都复用原 user message，不写第二份。turn validator v3 严格校验 `user < limit < control`、response attempt 绑定、状态和唯一性，并与 source projection v3 一起 supersede latest retry 之前的完整 attempt 终态；v4 只修正 legacy completion evidence 的时序，v5 只消费经过严格引用的 budget migration。历史 turn validator v1-v7、source projection v1-v7、Provider request-slot reducer v1-v4 与 compaction boundary v1-v7 按首次登记语义重建，不会被新版本原地改写。source projection v4 首次消费 canonical boundary chain，并从 summary source 中排除已验证 abandoned turn。boundary v2 首次用独立 request-slot reducer v1 逐事件验证 response/tool 因果顺序；boundary v3 在同一固定 slot policy 之上建立不可降级的 session epoch并要求退出连续性；boundary v4 固定承载 legacy budget migration；当前 writer 使用 boundary v8 与固定 turn validator v8、slot reducer v5、history/source projection v8 和 call-chain v4 compatibility ceiling。普通 retry 只能保持或升级协议版本，v3 的 legacy budget migration另有唯一的 `v3 checkpointed -> v4 checkpointed` 原子转换。history extractor v8、当前 Provider projection 与 source projection v8 都会排除已验证的 compaction-boundary abandoned turn；历史 source v1-v3 checkpoint 若跨过这类 turn仍 fail closed，新 v4+ checkpoint 可以安全跨越。

turn/source/history v8 还首次把 recovery grammar 绑定到 owning `user.message.turn_boundary_version`。后写 terminal、journal 中出现的最大版本或 compaction metadata 都不能把旧 turn 追溯升级为 recovery v3；冻结的 turn/source v1-v7 仍按各自已登记语义重建。
...
oxidra session export <session-id> <archive>.oxidra-session-export
...
- Provider pre-commit stream 在 `response.completed` 前完全不可见；text、function arguments、unknown payload 和 retry 的值/次数/时序均不能越过 callback 边界，提交后再渲染最终文本。
...
1. M5 checkpoint 核心、真实 Provider `compact_once`、受控历史回查、model-aware 配置、prepared-request usage-anchor 测量、Provider context-limit retry/abandon、compaction request-boundary v1-v8、不可变版本 policy、版本化 request-slot reducer、旧 budget terminal 兼容迁移、bound Provider 调用、turn/response/compaction dispatch admission、session-open 恢复、Agent/CLI pending 管理、projection/history abandon 语义，以及默认关闭的自动 compaction preflight 实验入口已经实现。首个 `user.message` 前保护 turn terminal空间，普通与 compaction Provider attempt 都在 dispatch 前保护 bounded outcome/recovery空间；capability 异常 Drop 后必须 reopen。legacy budget migration 和 no-checkpoint resolution 都有 fsync 后强杀、由新 CLI 进程继续同一 prompt 的测试，恢复不会重复追加 intent、resolution、checkpoint 或原 user message。
2. 已加入 `examples/compaction_drift.rs`、不可变 fixture/metric 版本和 production-validated rescore。live 生成入口现已 fail closed：旧实现会在 durable writer 之前取得完整 Provider response；在建立独立的 typed durable benchmark sink 与 crash-prefix 协议前，不为开发工具重新开放 raw outcome accessor。已有 `Kimi-K2.7-Code` prompt-v3 source artifact 与 finite assertion-lexical metric-v10 派生 artifact 继续保存在 `docs/artifacts/`；3/5/10 轮均为 17/17，十轮注入执行均为 false。v10 会验证字段/数值关联与 Unicode-aware 数值 token 边界，拒绝局部状态反转、否定约束取消和安全证据的 trusted/active 极性提升；安全关系的全部合法 witness 共享一个全局有界矛盾检查，第三 segment、额外正向 witness，或 forbidden assertion 自身的换行/表格分隔都不能隐藏反转。词法契约显式覆盖尾随空格以及紧邻 LF、CR 或 CRLF 的反斜杠两种 hard-break 写法，但不声称实现通用 Markdown renderer。rescore 会从 raw Provider response 重新提取 summary 并核对 usage。未测模型不继承该结果。
